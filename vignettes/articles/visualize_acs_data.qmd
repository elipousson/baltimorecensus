...
select_area_name <- "John Ruhrah EMS Area"
...
```{r}
geography_levels <- c(select_area, select_counties, select_msa)
...
area_summary_data <- combine_acs %>%
  ...
  left_join(pull_table(combine_acs,
    table = "B25003",
    vars = "B25003_001"
  ) |>
    select(
  ...
  left_join(
    pull_table(combine_acs, table = "B01003") |>
      select(
        name,
...
area_summary_data |>
  ...
  gt::tab_source_note("Source: 2017-2021 ACS 5-year Estimates")
...
if (params$save_plot) {
  papersize::ggsave_ext(
    filename = glue::glue("{janitor::make_clean_names(select_area_label)}_tenure_plot.pdf"),
    fileext = "pdf",
    paper = "Albert",
...
```{r}
combine_acs |>
  pull_table(table = c("B19013", "B19013A", "B19013B", "B19013I")) |> #  "B19013C", "B19013D", "B19013E", "B19013F", "B19013G", "B19013H",
  # View()
  bind_race_category_col() |> 
  rename(
    median_income_estimate = estimate,
...
combine_acs |>
  ...
  filter(name %in% c("John Ruhrah EMS Area", "Baltimore City")) |>
  pivot_wider(
    names_from = "name",
